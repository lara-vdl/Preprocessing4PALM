...
def readAndClipLayers(filename:str, layername:str, clipper:gpd.GeoDataFrame, epsg = 25832) -> gpd.GeoDataFrame:
    ...
    '''
    gdf = gpd.read_file(os.path.join(alkis_path,filename), layer=layername).set_crs(epsg)
...
def vectoriseImperviousness(rasterDs:str, clipper:gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame,gpd.GeoDataFrame]:
    '''
...
alkis_palm_all["land_use"] = alkis_palm_all["vegetation"]
alkis_palm_all["land_use"] = np.where(alkis_palm_all["water"].notna(), alkis_palm_all["water"] + 20, alkis_palm_all["land_use"])

# rasterise both layers
rasteriseSurface(alkis_palm_all,"pavement",2.0)
rasteriseSurface(alkis_palm_all,"land_use",2.0)